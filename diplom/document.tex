...
двух уравнений первого порядка.
  ...
  шагом
	...
	Рассмотрим дифференциальное уравнение \eqref{eq:9}. Пусть \(t_0\) и \(y_0\) это
	числа, а для \(g(t,y)\) существуют непрерывные частные производные до некоторого
	порядка \(n-1\). В таком случае для решения уравнения \eqref{eq:9} существуют
	...
	Тогда из разложения \eqref{eq:11}, выбросив члены \(o(|t-t_0|^{n+1})\), получаем
	\begin{equation}\label{eq:12}
		y(t_0+h)\cong y_0+hy_0^{(1)}+\frac{h^2}{2!}y_0^{(2)}+\ldots+\frac{h^n}{n!}y_0^{(n)}
	\end{equation}
	Производные из \eqref{eq:12} можно вычислить, однако получаемые формулы получаются слишком громоздкими. Поэтому К. Рунге предложил, а В. Кутта развил, идею вместо решения формулы \eqref{eq:12} решать следующие линейные комбинации
